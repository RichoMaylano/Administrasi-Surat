...
$title = "Surat Rekomendasi | Administrasi";
include '_header.php';
?>
...
<div class="row">
    ...
    <div class="card shadow mb-4" style="width:80%; left:10%">
      <div class="card-header bg-success bg-gradient text-white py-3">
		<div class="row">
		<div class="col-sm-10">
			<h4 class="m-0 font-weight-bold"><i class="fa-solid fa-envelope-circle-check"></i>  <strong>Surat Rekomendasi</strong></h4>
		</div>
		<div class="col-sm-2">
				<div class="btn-group shadow rounded">
					<!-- Button trigger modal -->
					<a class="btn btn-light" type="button" data-bs-toggle="modal" data-bs-target="#add_rekomendasi"aria-current="page">Tambah Data</a>
					<a href="#" class="btn btn-success"><i class="fa-regular fa-square-plus"></i></a>
				</div>
		</div>
		</div>
      </div>    
...
<!-- Modal Tambah Data-->
<div class="modal fade" tabindex="-1" id="add_rekomendasi" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header bg-success text-white">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fa-regular fa-square-plus"></i> &nbspTambah Data <strong>Surat Rekomendasi</strong></h5>
	  ...
	  <form action="add_proses_rekomendasi.php" class="form-horizontal" method="post">
        ...
        <h6 class="m-0 font-weight-bold mb-3">1. Nomor Surat</h6>
	     <div class="form-group row">
            ...
            <div id="emailHelp" class="form-text text-danger"><strong>Contoh : 420.2/958 (Inputkan Nomor Surat 958)</strong></div>
			</div>
        </div>
        ...
        <h6 class="m-0 font-weight-bold mb-3">2. Nama Lengkap</h6>
	     <div class="form-group row">
			<div class="form-group mb-3">
			<select id="nama_siswa" class="form-control" name="nama_lengkap">
...
<!-- Table -->
			...
			if(mysqli_num_rows($qsiswa) > 0){
				while($data = mysqli_fetch_array($qsiswa)){
					...
					Aksi
					 ...
					 <li><a href="../cetak_rekomendasi.php?no_surat='.$data['no_surat'].'"><button class="btn btn-sm"><i class="fas fa-download"></i> Unduh</button></a> </li>
					 <li><a data-bs-toggle="modal" data-bs-target="#edit_rekomendasi'.htmlspecialchars($data['id_surat']).'" class="btn btn-sm"><i class="fas fa-edit"></i> Edit</button></a></li>
					 <li><a data-bs-toggle="modal" data-bs-target="#delete_rekomendasi'.htmlspecialchars($data['id_surat']).'"  class="btn btn-sm"><i class="fas fa-trash"></i> Hapus</a></li>
					 </ul>
					 </div></td>';
	...
	<!-- Modal Edit Data-->
<div class="modal fade" tabindex="-1" id="edit_rekomendasi<?php echo htmlspecialchars($data['id_surat']) ?>" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header bg-warning text-white">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fa-solid fa-pen-to-square"></i> &nbspEdit Surat Rekomendasi <strong>420.2/<?php echo $data['no_surat']?> - <?php echo $data['nama_lengkap']?></strong></h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
...
<!-- Modal Delete Data-->
<div class="modal fade" id="delete_rekomendasi<?php echo htmlspecialchars($data['id_surat']) ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
	...
	<div class="modal-footer">
        ...
        <a href="delete_proses_rekomendasi.php?id_surat=<?php echo $data['id_surat']?>" type="submit" name="submit" class="btn btn-success"><i class="fas fa-check"></i> Yakin</a>
	</div>
    </div>
  </div>
</div>
...
<script>
		$(function() {
			$("#nama_siswa").change(function(){
				var nama_lengkap = $("#nama_siswa").val();
				...
				$.ajax({
					...
					success: function (data_siswa) {
						...
						$("#nama_ortu").val(data_siswa['nama_ibu']);
							...
							$("#nama_ortu").val(data_siswa['nama_ayah']);
						}
					}
				});
			});
